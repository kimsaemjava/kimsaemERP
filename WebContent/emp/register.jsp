...
<body>
<%
ArrayList<deptDTO> user = (ArrayList<deptDTO>) request.getAttribute("deptlist"); 
deptDTO deptlist= new deptDTO();
%>
						...
						<div class="col-sm-3">
							<select name="deptno" class="form-control" >
								<%for(int i=0; i<user.size();i++){
									deptlist=user.get(i);
								%>
								<option value="<%=deptlist.getDeptno() %>"><%=deptlist.getDeptname() %>
								<%}  %>
							</select>
						</div>
					</div>
