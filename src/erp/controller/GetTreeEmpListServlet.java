...
//조회된 정보를 jsp로 response할 때 비동기로 처리된 요청이므로 forward할 수 없고
//조회된 정보를 json으로 변환해서 response해야한다.
...
public class GetTreeEmpListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		int size = emplist.size(); // db에서 조회한 레코드의 갯수만큼 for문을 돌리면서 작업
		for (int i = 0; i < size; i++) {
			//ArrayList에서 하나 꺼낸 MemberDTO를 JSONObect로 변환
			...
			//db레코드 ->dto변환 ->JSONObject로 변환
			empjsonlist.add(empjson);
		}
		//변형된 JSONArray를 마지막으로 root인 JSONObject에 추가한다.
