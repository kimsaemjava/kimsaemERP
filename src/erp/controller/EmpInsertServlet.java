...
public class EmpInsertServlet extends HttpServlet {
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		while(files.hasMoreElements()){ // 파일이 존재하면
			...
			fileName = multipart.getFilesystemName(file); // 시스템에 저장된 이름. db에서 이걸 써야함 오리지널 ㄴㄴ
		}
		...
		//String profile_photo = multipart.getParameter("userImage");
		...
		MemberDTO user = new MemberDTO(id, pass, name, ssn, 
				...
				detailaddr,phonehome, phoneco, phonecell, email, fileName);
