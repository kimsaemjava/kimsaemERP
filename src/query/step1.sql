...
1. 로그인시 필요한 데이터를 조인해서 가져오기
	...
	=> 로그인 성공하면 content.jsp의 문자열을 job_category값으로 연결
	   로그인 성공 후 각 job_id별로 정의된 manupath에서 view정보를 가져와 연결하기
	   로그인 성공 후에 profile_photo에 등록된 이미지로 연결
	   컬럼이 달라졌으므로 
	   	LoginServlet, LoginDAOImpl, LoginDTO를 생성

select * from member where id = '9401023jang';

select * from member;

select m.*, d.deptname, j.job_category, j.menupath 
from member m, dept d, job j 
where m.deptno = d.deptno and d.job_category = j.job_id and id='9401023jang' and pass='1234';
